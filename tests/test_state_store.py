...
class TestStateStoreAgent:
    ...
    async def test_initialization(self, state_store: StateStoreAgent):
        ...
        expected_tables = ["health_metrics", "user_profiles", "daily_plans", "chat_messages"]
    ...
    async def test_store_and_retrieve_user_profile(
        ...
        assert retrieved_profile.prefer_turkish_cuisine == test_user_profile.prefer_turkish_cuisine
    ...
    async def test_date_range_filtering(
        ...
        # Test limit
        limited_metrics = await state_store.get_health_metrics(
            "test_user", 
            limit=2
        )
