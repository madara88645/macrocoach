...
def show_dashboard(user_id: str):
    ...
    try:
        ...
        if response.status_code == 200:
            ...
            if any(
                [
                    daily_summary.get("protein_g"),
                    daily_summary.get("carbs_g"),
                    daily_summary.get("fat_g"),
                ]
            ):
            ...
            if progress:
                ...
                with col1:
                    st.metric(
                        "Avg Calories/Day", f"{progress.get('avg_kcal_in', 0):.0f}"
                    )

                with col2:
                    st.metric("Avg Steps/Day", f"{progress.get('avg_steps', 0):,.0f}")

                with col3:
                    workout_rate = (
                        progress.get("workout_days", 0)
                        / progress.get("total_days", 1)
                        * 100
                    )
                    st.metric("Workout Rate", f"{workout_rate:.0f}%")

                # Weight trend
                weight_trend = progress.get("weight_trend", "stable")
                trend_emoji = {"increasing": "📈", "decreasing": "📉", "stable": "➡️"}
                st.info(
                    f"Weight trend: {trend_emoji.get(weight_trend, '➡️')} {weight_trend}"
                )
        ...
        st.info(
            "Please start the API server first: `poetry run uvicorn src.macrocoach.main:app --reload`"
        )
...
def show_chat_interface(user_id: str):
    ...
    if "messages" not in st.session_state:
        st.session_state.messages = [
            {
                "role": "assistant",
                "content": "Hi! I'm MacroCoach. Ask me about your progress or nutrition plan! 🏃‍♂️",
            }
        ]
    ...
    if prompt := st.chat_input("Type your message here..."):
        ...
        try:
            response = requests.post(
                f"{API_BASE_URL}/chat", json={"message": prompt, "user_id": user_id}
            )
    ...
    with st.form("data_entry"):
        ...
        with col1:
            ...
            calories = st.number_input(
                "Calories consumed", min_value=0, max_value=5000, step=50
            )

        with col2:
            protein = st.number_input(
                "Protein (g)", min_value=0.0, max_value=300.0, step=1.0
            )
            workout_type = st.selectbox(
                "Workout Type", ["None", "Strength", "Cardio", "Walking", "Running"]
            )
