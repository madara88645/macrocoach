...
from ..core.models import UserProfile, DailyPlan, HealthMetric
...
class PlannerAgent:
    ...
    def calculate_bmr(self, profile: UserProfile, weight: Optional[float] = None) -> float:
        """
    ...
    def calculate_macro_targets(
        ...
        total_percent = profile.protein_percent + profile.carbs_percent + profile.fat_percent
    ...
    async def generate_daily_plan(
        ...
        trend_adjustment = self.get_weight_trend_adjustment(recent_weights, profile.target_weight_kg)
        ...
        if goal_adjustment != 0:
            reasoning_parts.append(f"Goal adjustment: {goal_adjustment:+d} kcal ({profile.goal})")
            adjustments_made.append(f"Goal-based: {goal_adjustment:+d} kcal")
        
        if trend_adjustment != 0:
            reasoning_parts.append(f"Weight trend adjustment: {trend_adjustment:+d} kcal")
    ...
    def analyze_progress(
        ...
        for metric in recent_metrics:
            ...
            if date_str not in daily_summaries:
                daily_summaries[date_str] = {
                    "kcal_in": 0, "kcal_out": 0, "protein": 0,
                    "steps": 0, "weight": None, "workouts": 0
                }
        ...
        avg_kcal_in = sum(s["kcal_in"] for s in daily_summaries.values()) / len(daily_summaries)
        avg_steps = sum(s["steps"] for s in daily_summaries.values()) / len(daily_summaries)
        avg_protein = sum(s["protein"] for s in daily_summaries.values()) / len(daily_summaries)
        ...
        return {
            ...
            "workout_days": sum(1 for s in daily_summaries.values() if s["workouts"] > 0),
