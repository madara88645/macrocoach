...
class ChatUIAgent:
    ...
    async def _handle_status_command(self, user_id: str) -> str:
        ...
        if summary["total_metrics"] == 0:
            return (
                "📊 No data logged for today yet. Use /add to log your first metrics!"
            )
        ...
        if profile:
            recent_metrics = await self.state_store.get_health_metrics(
                user_id, start_date=today - timedelta(days=7), limit=50
            )
    ...
    async def _handle_plan_command(self, user_id: str) -> str:
        ...
        if not profile:
            return (
                "🏃‍♂️ I need your profile first! Please tell me:\n"
        ...
        recent_metrics = await self.state_store.get_health_metrics(
            user_id, start_date=datetime.now() - timedelta(days=14), limit=50
        )

        # Generate plan
        tomorrow = datetime.now() + timedelta(days=1)
        plan = await self.planner.generate_daily_plan(
            user_id, profile, recent_metrics, tomorrow
        )
    ...
    async def _handle_add_command(self, user_id: str, data: str) -> str:
        ...
        metric = HealthMetric(
            timestamp=datetime.now(), user_id=user_id, source="manual"
        )
        ...
        if confirmations:
            ...
            return (
                "❌ Couldn't parse your data. Try formats like:\n"
                "• 'weight 70.5 kg'\n"
                "• '8500 steps'\n"
    ...
    async def _handle_natural_language(self, message: str, user_id: str) -> str:
        ...
        if any(
            ...
            for word in ["status", "how am i", "progress", "today"]
        ):
            return await self._handle_status_command(user_id)

        elif any(
            word in message_lower for word in ["plan", "tomorrow", "what should i eat"]
        ):
            return await self._handle_plan_command(user_id)
